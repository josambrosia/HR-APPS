...
from src.config import SEVERE_LATENESS_CATEGORIES
from src.db.connection import get_connection
from src.db.settings import get_setting, read_severe_lateness_threshold
from src.db.attendance import (
    set_reason, unresolve_issue,
...
class SevereLatenessScreen(ctk.CTkFrame):
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(header, text="Severe Lateness",
                     font=FONT_DISPLAY,
    ...
    def _on_batch_resolve(self):
        ...
        BatchResolveDialog(
            ...
            lister_fn=lambda conn, s, e: list_severe_lateness_for_period(
    ...
    def _render_stats(self, counts):
        ...
        rate_str = f"{rate}%" if total > 0 else "—"
    ...
    def _build_panel_for(self, row_data):
        ...
        labels = [REASON_LABELS[c] for c in SEVERE_LATENESS_CATEGORIES]
    ...
    def _reload(self):
        ...
        with get_connection(DB_PATH) as conn:
            counts = count_severe_lateness_for_period(conn, start, end, self._threshold)
            open_rows = list_severe_lateness_for_period(
                conn, start, end, self._threshold, resolved=False)
            resolved_rows = list_severe_lateness_for_period(
                conn, start, end, self._threshold, resolved=True)
