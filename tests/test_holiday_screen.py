...
def test_holiday_on_show_picks_up_new_month_and_data(temp_db_path, monkeypatch, tk_root):
    """Built with no active month (empty state) — when a month with data
    appears later, on_show must render the roster and show the badge."""
    import src.ui.screens.holiday as holiday_mod
    monkeypatch.setattr(holiday_mod, "DB_PATH", temp_db_path)
    init_db(temp_db_path)
    screen = holiday_mod.HolidayScreen(tk_root)
    tk_root.update_idletasks()
    assert screen._rows == {}
    assert screen._badge.winfo_manager() == ""  # badge hidden without month
    with get_connection(temp_db_path) as conn:
        a = upsert_employee(conn, no_staff="1", nama="ANDI", dept="X")
        upsert_attendance(
            conn, employee_id=a, tanggal="2026-04-01", hari="Senin",
    ...
    screen.on_show()
    tk_root.update_idletasks()
    assert screen._month == "2026-04"
    assert "2026-04-01" in screen._rows
    assert screen._badge.winfo_manager() == "pack"  # badge now shown
    screen.destroy()
